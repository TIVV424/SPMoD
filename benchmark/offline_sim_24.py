...
## Create driver with time
# driver["time"] = pd.date_range(start=start_time - pd.Timedelta(minutes=15), end=start_time - pd.Timedelta(minutes=15), periods=len(driver))
...
print(" =============== Running time ============== \n", end_time - start_time)
print(match_result)
np.save("Database//online_result_0602//match_result_weight_%s.npy" % str(weight_on), match_result)

pd.DataFrame({"match_result": match_result, "time": end_time - start_time, "weight": weight_on}, index=[0]).to_csv(
    "Database//online_result_0602//match_result_weight_%s.csv" % str(weight_on)
)
